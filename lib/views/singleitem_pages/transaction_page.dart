...
class TransactionPage extends StatefulWidget {
  final t.Transaction item;
  TransactionPage(this.item);
  @override
  State<StatefulWidget> createState() => _TransactionPageState(item);
}
...
class _TransactionPageState extends State<TransactionPage> {
  ...
  _TransactionPageState(this.item);
                      ...
                      children:[ 
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(item.type == 1 ? "Income" : "Expense", style: TextStyle(color: Colors.white70, fontSize: 18.0),),
                              Padding(
                                padding: const EdgeInsets.all(8.0),
                                child: Text(format(item.money), style: TextStyle(color: Colors.white, fontSize: 48.0, fontWeight: FontWeight.w500),),
                              ),
                            ],
                          ),
                        ),
                      ...
                      child: Row(
                        ...
                        children: [
                          Container(
                            padding: EdgeInsets.all(16.0),
                            decoration: BoxDecoration(
                              color: StringConts.categories[item.category]['color'].withOpacity(0.1),
                              borderRadius: BorderRadius.circular(16.0)
                            ),
                            child: Icon(StringConts.categories[item.category]['icon'], color: StringConts.categories[item.category]['color'], size: 24.0,),
                          ),
                          Padding(
                            ...
                            child: Column(
                              ...
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Category", style: TextStyle(fontWeight: FontWeight.w400, fontSize: 14.0),),
                                Text(item.category, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 16.0),),
                              ],
                            )
                          )
                        ],
  ...
  void _edit(t.Transaction item){
    Navigator.of(context).pop();
    Navigator.push(context, MaterialPageRoute(builder: (context){
      return MainPage();
    }));
    Navigator.push(context, MaterialPageRoute(builder: (context) {
      return TransactionsFormsPage(item, "Edit", currency, item.type);
    }));
  }
  void _delete(t.Transaction item) async{
    int result = await databaseHelper.deleteTransaction(item.id);
